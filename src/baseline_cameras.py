...
def calculate_reprojection_error(homography, line_matches):
    """
    Calculates the reprojection error for the given homography and line matches.
    :param homography: The homography matrix.
    :param line_matches: List of pairs of 2D lines matches.
    :return: The reprojection error.
    """
    total_error = 0
    for line_pair in line_matches:
        src_line = line_pair[0]
        target_line = line_pair[1]

        reprojected_line = np.transpose(np.linalg.inv(homography)) @ src_line
        reprojected_line /= np.linalg.norm(reprojected_line[:2])
        target_line /= np.linalg.norm(target_line[:2])
        error = np.abs(np.dot(reprojected_line, target_line))
        total_error += error

    return total_error / len(line_matches)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--images', default="/home/fmg/data/SN23/calibration-2023-bis/", type=str,
    ...
    dataset_dir = os.path.join(args.images)
    ...
    prediction_files = [f for f in sorted(
        os.listdir(args.prediction)) if ".json" in f]

    with tqdm(enumerate(prediction_files), total=len(prediction_files), ncols=160) as t:
        for frame in prediction_files:
            ...
            # image_path = os.path.join(dataset_dir, frame.stem + ".jpg")
            ...
            for k, v in predictions.items():
                if 'Circle' in k or "unknown" in k or 'Goal' in k:
                    continue
            ...
            if len(line_matches) >= 4:
                ...
                reprojection_error = calculate_reprojection_error(
                    homography, line_matches)
                homography_file = os.path.join(
                    args.prediction, f"homography_{frame_index}.json")
                homography_dict = {"success": int(success),
                                   "reprojection_error": reprojection_error,
                                   "homography": homography.tolist()}
                with open(homography_file, 'w') as f:
                    json.dump(homography_dict, f, indent=4)
                if success:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            task2_prediction_file = os.path.join(
                                args.prediction, f"camera_{frame_index}.json")
